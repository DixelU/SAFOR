...
struct OverlapRemover{
	...
	list<ULI, moya_alloc::allocator<ULI,100000>> *PNO;//first 128 is first channel, next 128 are the second... etc//quite huge boi 
	bbb_ffr *fin;
	OverlapRemover(){
		RSB=PPQN=CTrack=0;
		PNO=new list<ULI, moya_alloc::allocator<ULI,100000>>[2048];
	}
	...
	ULI FindAndPopOut(LTE pos,ULI CTick){
		list<ULI, moya_alloc::allocator<ULI,100000>>::iterator Y=PNO[pos].begin();
		ULI q=PNO[pos].size();
		/*
